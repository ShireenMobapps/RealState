//
//  TenantNotificationsVC.swift
//  AIPoweredRealEstate
//

import UIKit

final class TenantNotificationsVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        TenantProfileStyle.applyPushed(self, title: "Notifications")
    ...
    private func loadPage(reset: Bool) {
        ...
        Task {
            do {
                let response = try await AuthViewModel.notificationsAPI(page: requestPage, limit: limit)
                await MainActor.run {
    ...
    private func deleteNotification(id: String) {
        ...
        Task {
            do {
                _ = try await AuthViewModel.deleteNotificationAPI(id: notificationId)
                await MainActor.run {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let leadId = items[indexPath.row].leadId?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !leadId.isEmpty else { return }
        openBuyerLead(id: leadId)
    }
